...
export class AuthService {
  ...
  async sendPasswordReset(email: string): Promise<{ message: string }> {
    ...
    try {
      await this.mailService.sendPasswordResetEmail(email, resetUrl, user.firstName);
      return { 
        message: 'Si el correo existe, recibirás las instrucciones para restablecer tu contraseña' 
      };
    } catch (error) {
  ...
  async resetPassword(token: string, newPassword: string): Promise<{ message: string }> {
    try {
      ...
      try {
        await this.mailService.sendPasswordChangedNotification(user.email, user.firstName);
      } catch (error) {
